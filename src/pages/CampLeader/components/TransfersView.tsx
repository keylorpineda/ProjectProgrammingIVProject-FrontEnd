      ...
      <div className="flex flex-col gap-4 bg-black/40 p-6 border border-[#3b4d3e] rounded">
        ...
        <div className="flex flex-wrap bg-[#111111] p-1 rounded border border-[#3b4d3e]/40 gap-1">
          <button
            ...
            className={`px-3 py-1 font-mono text-xs uppercase font-bold tracking-wider rounded cursor-pointer ${
              filterRole === "ALL"
                ? "bg-[#c27c2f] text-black font-semibold"
                : "text-zinc-400 hover:text-[#fca311]"
            }`}
          >
            TODOS LOS CONVOYES
          </button>
          <button
            ...
            className={`px-3 py-1 font-mono text-xs uppercase font-bold tracking-wider rounded cursor-pointer ${
              filterRole === "origin"
                ? "bg-[#3b4d3e] text-white font-semibold"
        ...
        {filteredTransfers.length === 0 ? (
          ...
          filteredTransfers.map((t) => {
            ...
            const originCampName =
              camps.find((c) => c.id === t.origin_camp_id)?.name || "CAMPAMENTO DESCONOCIDO"
            const destCampName =
              camps.find((c) => c.id === t.destination_camp_id)?.name || "CAMPAMENTO DESCONOCIDO"
              ...
              >
                ...
                <div className="border-b border-black/10 pb-3 mb-3">
                  <div className="flex justify-between items-start gap-2">
                    <span className="font-mono text-xs font-bold text-zinc-700 block uppercase tracking-wider">
                      TRASLADO #{t.id}
                    </span>
                    <span
                      className={`px-2 py-1 font-mono text-[11px] font-bold rounded uppercase border shrink-0 ${
                        t.status === "pending"
                          ? "bg-amber-200 text-amber-900 border-amber-600 animate-pulse"
                          : t.status === "in_transit"
                            ? "bg-amber-500 text-black border-black animate-pulse"
                            : t.status === "approved"
                              ? "bg-[#4c6351] text-white border-black"
                              : t.status === "completed"
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <div className="bg-black/10 w-full p-2 rounded border border-black/10 text-xs font-mono flex items-center justify-between gap-2">
                      <div
                        className="truncate text-left w-1/3 text-zinc-850 font-bold"
                        title={originCampName}
                      >
                        {isOriginUs ? "NUESTRO BÚNKER" : originCampName.substring(0, 15) + "..."}
                      </div>
                      <ArrowRight className="w-3.5 h-3.5 text-zinc-900 shrink-0" />
                      <div
                        className="truncate text-right w-1/3 text-zinc-850 font-bold"
                        title={destCampName}
                      >
                ...
                <div className="border-t border-black/10 pt-3 mt-auto">
                  {/* A: Pending + destino somos nosotros → aprobar/rechazar */}
                  {t.status === "pending" && !isOriginUs && (
                    <div className="flex gap-2">
                      <button
                        onClick={() => onApproveTransferRequest(t.id, true)}
                        className="flex-1 bg-[#4c6351] text-white py-2.5 px-3 hover:bg-[#3b4d3e] cursor-pointer text-xs font-typewriter font-bold uppercase border-2 border-black flex items-center justify-center gap-1.5"
                      >
                      ...
                      <button
                        ...
                        className="bg-red-800 hover:bg-red-700 text-white py-2.5 px-3 cursor-pointer border-2 border-black flex items-center gap-1.5 text-xs font-typewriter font-bold uppercase"
                      >
                  ...
                  {/* B: Pending + origen somos nosotros → cancelar */}
                  {t.status === "pending" && isOriginUs && (
                    <button
                      onClick={() => onCancelTransferRequest(t.id)}
                      className="w-full bg-red-800 hover:bg-red-700 text-white font-typewriter text-xs py-2.5 px-3 border-2 border-black cursor-pointer uppercase flex items-center justify-center gap-1.5"
                    >
                      <X className="w-3.5 h-3.5" />
                      CANCELAR SOLICITUD
                    </button>
                  ...
                  {/* C: En tránsito + destino somos nosotros → confirmar llegada */}
                  {(t.status === "in_transit" || t.status === "approved") && !isOriginUs && (
                    <button
                      onClick={() => onArriveTransferRequest(t.id)}
                      className="w-full bg-amber-500 hover:bg-amber-400 text-black font-typewriter text-xs font-bold py-2.5 px-3 border-2 border-black cursor-pointer flex items-center justify-center gap-1.5 uppercase"
                    >
                  ...
                  {(t.status === "in_transit" || t.status === "approved") && isOriginUs && (
                    <div className="text-center py-2 bg-amber-950/20 text-[#8f581e] font-typewriter text-xs uppercase font-bold tracking-wider rounded border border-[#8f581e]/40">
                      {"● CONVOY EN RUTA — AGUARDANDO CONFIRMACIÓN"}
                    </div>
                  )}

                  {/* Scenario E: Suministros Completados */}
                  {t.status === "completed" && (
                    <div className="text-center text-zinc-500 font-typewriter text-xs uppercase font-bold py-2 border border-zinc-500/20 rounded">
                      ✓ ENTREGADO — ARCHIVADO
                    </div>
                  )}

                  {t.status === "rejected" && (
                    <div className="text-center text-red-800 bg-red-500/10 font-typewriter text-xs uppercase font-bold tracking-wider border border-red-500/25 rounded py-2">
                      ✗ TRASLADO RECHAZADO
                    </div>
                  )}

                  {t.status === "cancelled" && (
                    <div className="text-center text-zinc-500 font-typewriter text-xs uppercase font-bold tracking-wider py-2">
                      — CONVOY CANCELADO —
                    </div>
                  )}
                </div>
