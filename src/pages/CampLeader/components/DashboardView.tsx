...
export default function DashboardView({
  ...
  const criticalStocks = inventory.filter(i => 
    i.alert_active && (i.resource.category === 'food' || i.resource.category === 'water')
  );
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <motion.div 
          ...
          className="bg-[#9a9080] border border-black p-5 relative overflow-hidden text-black transition-transform hover:scale-[1.01] flex flex-col justify-between group h-48"
          style={{ transform: 'rotate(0.4deg)' }}
        >
          <div className="flex justify-between items-start">
            <span className="font-mono text-[10px] uppercase font-bold text-zinc-800 tracking-wider">
              [EQUIPOS EN ZONA MUERTA]
            </span>
            <span className="p-1 px-2 text-[9px] font-mono font-bold bg-amber-950/20 text-amber-900 border border-amber-900/40 rounded">
              ZONA NEGRA
            </span>
          </div>
          ...
          <div className="my-3">
            ...
            <span className="font-mono text-[11px] text-zinc-900 uppercase font-medium">
              EQUIPOS ACTIVOS RASTREANDO
            </span>
          </div>

          <div className="text-[10px] font-mono text-zinc-800 border-t border-black/20 pt-2 flex items-center justify-between">
        ...
        <motion.div 
          ...
          className="bg-[#9a9080] border border-black p-5 relative overflow-hidden text-black transition-transform hover:scale-[1.01] flex flex-col justify-between group h-48 bg-amber-100/90"
        ...
        >
          <div className="flex justify-between items-start">
            ...
            <span className="p-1 px-2 text-[9px] font-mono font-bold bg-zinc-950 text-white rounded animate-pulse">
              PENDIENTES
            </span>
          </div>
        ...
        <motion.div 
          ...
          className={`bg-[#9a9080] border border-black p-5 relative overflow-hidden text-black transition-transform hover:scale-[1.01] flex flex-col justify-between group h-48 ${
            criticalStocks.length > 0 ? 'warning-card text-[#9c2720]' : 'bg-emerald-200/95 text-[#2b4c33] border-emerald-900'
          }`}
          style={{ transform: 'rotate(-0.5deg)' }}
        >
          <div className="flex justify-between items-start">
            ...
            <AlertOctagon className={`w-4 h-4 ${criticalStocks.length > 0 ? 'animate-bounce' : ''}`} />
          </div>
          ...
          <div className="my-3">
            {criticalStocks.length > 0 ? (
              ...
              <>
                ...
                <span className="font-mono text-[10px] uppercase block opacity-85">
          ...
          <div className="text-[10px] font-mono border-t border-black/20 pt-2 flex items-center justify-between">
        ...
        >
          <div className="flex justify-between items-start">
            ...
            <span className="p-1 px-2 text-[9px] font-mono font-bold bg-[#3b4d3e] text-white rounded">
              SECTOR 1
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div id="active-teams-dashboard" className="bg-black/30 border border-[#3b4d3e] rounded-lg p-6 backdrop-blur-sm shadow-md lg:col-span-8">
          ...
          {activeExplorations.length === 0 ? (
            <div className="border border-dashed border-zinc-800 text-center py-10 rounded">
              ...
              <p className="font-mono text-[10px] text-amber-500 mt-1 cursor-pointer hover:underline" onClick={() => onNavigate('explorations')}>
              ...
              {activeExplorations.map((exp) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pl-3">
                      <span className="font-mono text-[10px] text-amber-500 font-bold tracking-widest block">
                        MISIÓN ID: #{exp.id} • {exp.departure_date.split('T')[0]}
                      </span>
                      <h4 className="font-typewriter text-sm text-white font-bold uppercase mt-0.5">
                        {exp.name}
                      </h4>
                      <p className="font-mono text-[11px] text-[#ab9e8b] mt-1">
                    ...
                    <div className="text-left md:text-right shrink-0">
                      <span className="font-mono text-[11px] text-[#ab9e8b] block">
        ...
        <div id="camp-balance-dashboard" className="bg-black/30 border border-[#3b4d3e] rounded-lg p-6 backdrop-blur-sm shadow-md lg:col-span-4">
          <div className="flex items-center gap-3 border-b border-[#c27c2f]/20 pb-3 mb-4">
          ...
          <div className="space-y-4">
            {balances.map((bal) => {
              ...
              return (
                <div key={bal.resource_id} className="border border-zinc-900 p-2.5 rounded bg-black/20">
                  <div className="flex justify-between items-center text-[11px] font-mono">
                    <span className="text-white font-bold uppercase">{bal.resource_name}</span>
                    <span className={`font-bold ${isPositive ? 'text-emerald-500' : 'text-red-500 animate-pulse'}`}>
                  ...
                  <div className="w-full h-1.5 bg-zinc-800 rounded mt-2 overflow-hidden flex">
                    <div 
                      ...
                      style={{ width: `${Math.min(100, (bal.consumption / (bal.production + bal.consumption || 1)) * 100)}%` }} 
                    />
                    <div 
                      className="bg-emerald-500 h-full"
                      style={{ width: `${Math.min(100, (bal.production / (bal.production + bal.consumption || 1)) * 100)}%` }} 
                    />
                  </div>
      ...
      <div id="recent-logs-dashboard" className="bg-black/45 border border-[#3b4d3e] p-4 font-mono text-[11px] leading-relaxed rounded overflow-hidden">
        <p className="text-[#3b4d3e] border-b border-[#3b4d3e]/40 pb-1 mb-2 font-bold">[ SISTEMA DE ESCUCHA LOGÍSTICA ]</p>
        <div className="flex items-center gap-3 border-b border-[#c27c2f]/20 pb-3 mb-4">
              ...
              {movements.slice(0, 5).map((mov) => {
                ...
                return (
                  <tr key={mov.id} className="text-zinc-300 hover:bg-black/40">
                    ...
                    <td className={`py-2.5 font-bold ${isAddition ? 'text-emerald-500' : 'text-red-500'}`}>
                      {isAddition ? `+${mov.quantity}` : `${mov.quantity}`}
                    </td>
                    <td className="py-2.5 text-[10px]">
                      <span className={`px-1.5 py-0.5 rounded uppercase ${
                        isAddition ? 'bg-emerald-950/40 text-emerald-400' : 'bg-red-950/40 text-red-400'
                      }`}>
                        {mov.type.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="py-2.5 text-[11.5px] text-zinc-400">
                      {mov.notes}
                    </td>
                  </tr>
                );
