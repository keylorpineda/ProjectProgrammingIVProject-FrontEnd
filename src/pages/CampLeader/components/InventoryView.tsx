...
import { Inventory, ResourceCategory } from '../types';
import { 
  Boxes, 
  Flame, 
  Droplet, 
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="border-b border-[#c27c2f]/30 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center">
        <div>
          ...
          <p className="font-mono text-xs text-[#fca311]/60 uppercase tracking-widest">
            AUDITORÍA HISTÓRICA DE RACIONES, ARMAS Y ELEMENTOS MEDICOS EN EL REFUGIO ALFA
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 bg-black/40 p-4 border border-[#3b4d3e] rounded">
        ...
        {[
          ...
          { id: 'medicine', label: 'Médicos e Inmunes', icon: HeartPulse },
          { id: 'tools', label: 'Repuestos Chatarra', icon: Wrench },
          { id: 'weapons', label: 'Munición Balística', icon: Sword }
        ].map((cat) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredInventory.map((inv) => {
          const ratio = Math.min(100, (inv.current_quantity / (inv.minimum_stock_required || 1)) * 100);
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[9px] font-mono font-bold tracking-widest text-zinc-600 block uppercase">
                ...
                {inv.alert_active ? (
                  <div className="flex items-center gap-1 bg-red-950 text-red-100 text-[9px] font-bold font-mono px-2 py-0.5 rounded border border-red-700 animate-pulse">
                    <AlertTriangle className="w-3 h-3 text-red-400" />
                    BODEGA BAJO MÍNIMO
                  </div>
                ) : (
                  <div className="bg-emerald-850/20 text-[#2c3d31] border border-[#2c3d31]/30 text-[9px] font-bold font-mono px-2 py-0.5 rounded">
                    SITUACIÓN SEGURA
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 font-mono">
                ...
                <div className="flex justify-between text-[10px] text-zinc-650">
                  <span>MÍNIMO REQUERIDO: {inv.minimum_stock_required} {inv.resource.unit}</span>
                  <span className="font-bold">{Math.round(ratio)}% SATURADO</span>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-black/5 flex justify-between items-center text-[10px] font-mono text-zinc-700">
                <span>VINCULACIÓN LOGÍSTICA COSTA GRIS</span>
