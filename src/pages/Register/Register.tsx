...
type RegisterStatus = "waiting" | "processing" | "granted" | "denied"
...
export default function Register() {
  ...
  const [registerStatus, setRegisterStatus] = useState<RegisterStatus>("waiting")
  const [isGateOpen, setIsGateOpen] = useState(false)
  const rafRef = useRef<number | null>(null)

  // MotionValues bypass React state
  ...
  useEffect(() => {
    if (!token) {
      navigate("/admissions/new", { replace: true })
    }
  }, [token, navigate])

  const finalizeRegistrationAndLogin = (role?: string) => {
    ...
    if (!destination) {
      ...
      setRegisterStatus("denied")
      setTimeout(() => setRegisterStatus("waiting"), 2000)
      return
    }

    setRegisterStatus("granted")
    setTimeout(() => {
      setIsGateOpen(true)
      setTimeout(() => {
  ...
  const handleRegister = async (u: string, p: string) => {
    ...
    try {
      ...
      finalizeRegistrationAndLogin(response.user.role)
    } catch {
      setRegisterStatus("denied")
      setTimeout(() => setRegisterStatus("waiting"), 2000)
    }
    ...
    <div className="industrial-login-page fixed inset-0 overflow-hidden bg-[#020202] font-sans selection:bg-[#4ade80] selection:text-black">
      ...
      >
        ...
        >
          ...
          <div className="w-[15%] h-full relative" style={{ transform: "translateX(-20%) scale(0.8)" }}>
      ...
      {/* Register Badge placed within the 3d environment */}
      <div className="relative z-50">
        <BadgeRegister
          onRegister={handleRegister}
          isProcessing={registerStatus === "processing"}
