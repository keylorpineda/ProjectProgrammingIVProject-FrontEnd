...
export function BadgeRegister({ onRegister, isProcessing, registerStatus }: BadgeRegisterProps) {
  ...
  const currentVariant = registerStatus === "granted" ? "granted" : 
                         registerStatus === "processing" ? "processing" :
                         (isHanging ? "hanging" : "initialDistortion");
    ...
    >
      <AnimatePresence>
        {isHanging && (
          <motion.div 
            ...
            style={{ backgroundImage: "linear-gradient(90deg, #2a251a 0%, #4a4538 50%, #2a251a 100%)" }}
          />
        )}
      </AnimatePresence>
      ...
      <div 
        ...
        style={{
          ...
          boxShadow: "inset 0 3px 3px rgba(255,255,255,0.8), inset 0 -3px 5px rgba(0,0,0,0.8), 0 5px 10px rgba(0,0,0,0.6)",
      ...
      <div style={{ filter: "drop-shadow(0 20px 25px rgba(0,0,0,0.8)) drop-shadow(0 5px 10px rgba(0,0,0,0.5))", width: "100%" }}>
        <div 
          ...
          style={{ 
            ...
            boxShadow: "inset 0 0 40px rgba(50,40,30,0.4), inset 2px 2px 10px rgba(255,255,255,0.6), 0 0 0 1px #222",
            clipPath: "polygon(0 8px, 4px 6px, 8px 0, 30% 0, 32% 4px, 35% 0, calc(100% - 8px) 0, 100% 8px, 100% 40%, 96% 42%, 100% 44%, 100% 80%, 97% 81%, 95% 83%, 98% 87%, 92% 89%, 95% 94%, 86% 96%, 82% 100%, 60% 100%, 58% 96%, 50% 100%, 8px 100%, 0 calc(100% - 8px), 0 60%, 4% 58%, 0 54%, 0 20%, 4% 16%, 1% 14%, 0 10%)",
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none z-10 rounded-xl border border-white/40" style={{ boxShadow: "inset 0 0 15px rgba(255,255,255,1), inset 0 0 30px rgba(255,255,255,0.5)" }} />
        ...
        <div className="absolute inset-0 pointer-events-none mix-blend-overlay opacity-30 z-0"
        ...
        <div className="flex-1 w-full flex flex-col p-4 sm:p-6 relative z-20" style={{ marginTop: "3.5rem" }}>
          
          {/* Header */}
          <div className="border-b-[4px] border-[#3a3528] pb-3 mb-10 flex justify-between items-start gap-2" style={{ width: "80%", margin: "0 auto" }}>
             <div className="flex flex-col relative w-full pt-1">
               <h2 className="font-sans font-black text-[20px] sm:text-[22px] tracking-tighter leading-none uppercase" style={{ color: "#3a3528" }}>FORJAR IDENTIDAD</h2>
               <p className="text-[10px] tracking-widest font-bold uppercase mt-1" style={{ color: "#3a3528" }}>Registro de Personal</p>
             </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-6 w-full" style={{ marginTop: "30px" }}>
            ...
            >
               <div className="w-[110px] h-[140px] shrink-0 bg-[#8c8270] border-2 border-[#1a1712] shadow-[inset_0_2px_10px_rgba(0,0,0,0.5),0_5px_8px_rgba(0,0,0,0.2)] flex items-center justify-center overflow-hidden relative sepia-[0.3]">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-4xl font-black text-black/40 mix-blend-overlay">?</div>
                  
                  <div className="absolute inset-0 shadow-[inset_0_0_15px_rgba(0,0,0,0.8)] pointer-events-none" />
                  <div className="absolute inset-0 pointer-events-none mix-blend-overlay opacity-30" style={{ backgroundImage: "radial-gradient(circle at center, rgba(0,0,0,0.6) 0%, transparent 100%)" }} />
               </div>

               <div className="flex-1 flex flex-col gap-3">
                 <motion.div className="flex flex-col gap-1.5 relative" animate={(shake && !username) || registerStatus === "denied" ? { x: [-8, 8, -6, 6, -4, 4, 0], transition: { duration: 0.4 } } : {}}>
                 ...
                 <motion.div className="flex flex-col gap-1.5 relative" animate={(shake && !password) || registerStatus === "denied" ? { x: [-8, 8, -6, 6, -4, 4, 0], transition: { duration: 0.4 } } : {}}>
                   <label className="text-[12px] font-black font-sans text-[#1a1a1a] tracking-[0.2em] uppercase flex items-center gap-2">
                     NUEVA CLAVE
                   </label>
                     ...
                     >
                        ...
                        >
                           <motion.div
                              ...
                              animate={{ height: showPassword ? 8 : 2, width: showPassword ? 8 : 12, opacity: showPassword ? 1 : 0.4 }}
                           />
                        </motion.div>
                     </button>
                   </div>
            ...
            <div className="w-[80%] flex justify-between mb-2 items-center gap-4" style={{ margin: "0 auto", marginTop: "3rem" }}>
               ...
               <div className="w-12 h-12 rounded-full border border-blue-500/30 flex items-center justify-center bg-gradient-to-tr from-blue-100 via-blue-400 to-blue-600 opacity-80 mix-blend-multiply shadow-[inset_0_0_15px_rgba(0,0,0,0.6)] relative overflow-hidden shrink-0">
                  <span className="text-[9px] font-black text-black/40 rotate-45 tracking-widest drop-shadow-md">NEW ID</span>
            ...
            <div className="flex flex-col gap-4 mt-4 mb-2 w-[80%]" style={{ margin: "0 auto" }}>
              <button 
                ...
                onMouseEnter={e => {
                  ...
                  (e.currentTarget as HTMLButtonElement).style.boxShadow = "inset 0 0 0 2px #3b82f6, 0 4px 0 #0a0a0a, 0 0 20px rgba(59,130,246,0.4), 0 5px 5px rgba(0,0,0,0.5)";
                  (e.currentTarget as HTMLButtonElement).style.color = "#3b82f6";
                }}
                onMouseLeave={e => {
                  (e.currentTarget as HTMLButtonElement).style.backgroundColor = "#1a1a1a";
                  (e.currentTarget as HTMLButtonElement).style.boxShadow = "inset 0 0 0 2px #333, 0 4px 0 #0a0a0a, 0 5px 5px rgba(0,0,0,0.5)";
              ...
              >
                ...
                <span className="relative z-10 group-active:text-blue-300 transition-colors">{isProcessing ? "VERIFICANDO..." : "ESTABLECER IDENTIDAD"}</span>
              </button>
            </div>
          </form>
        </div>
        ...
        <motion.div 
          ...
          animate={{ backgroundPosition: isHanging ? "50% 100%" : "50% 10%", opacity: isHanging ? 0 : 0.7 }}
          transition={{ duration: 1.5, ease: "easeOut" }}
          style={{
            background: "linear-gradient(105deg, transparent 30%, rgba(255,255,255,0.05) 45%, rgba(255,255,255,0.6) 48%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0.6) 52%, rgba(255,255,255,0.05) 55%, transparent 70%)",
            backgroundSize: "100% 300%"
          }} 
        />
        <div className="absolute inset-0 pointer-events-none z-40 border border-white/20" style={{ borderRadius: "16px" }} />
      </div>
      </div>
    </motion.div>
  );
